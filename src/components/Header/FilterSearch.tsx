...
function FilterSearch() {
    ...
    return (
        <div className="flex-1 max-w-[40rem] ml-2 md:ml-4 sm:self-stretch md:self-end md:pb-2 lg:pb-0 lg:self-auto flex justify-end">
            <div className={`${isEmpty?'w-12 rounded-full':'w-full rounded'} h-8 px-2 border-2 flex items-center bg-gray-600`}>
                <input className="w-full h-6 text-sm text-gray-200 bg-gray-600 focus:outline-none" value={filter} onChange={(event) => setFilter(event.target.value)} />
                {isEmpty
