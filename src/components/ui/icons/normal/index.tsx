export * from "./icon01-microsope";
export * from "./icon02-rocket";
export * from "./icon03-menu-burger";
export * from "./icon04-trash";
export * from "./icon05-close";
export * from "./icon06-back";
export * from "./icon07-chevrons";
export * from "./icon08-search";
export * from "./icon09-search-doc";
export * from "./icon10-preview";

export * from "./icon21-ctrl";
export * from "./icon22-case-regex";
export * from "./icon22-case-same-doc";
export * from "./icon22-case-same";
export * from "./icon22-case-sensitive";
export * from "./icon23-info";
export * from "./icon24-docs-accepted";
export * from "./icon25-doc";
export * from "./icon26-folder";

export * from "./icon30-apps";
export * from "./icon31-btn-form-login";
export * from "./icon32-btn-from-change";
export * from "./icon40-all-fields";
