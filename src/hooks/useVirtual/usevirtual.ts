...
const getInitItems = (itemSize: ItemSize, ssrItemCount?: SsrItemCount) => {
    ...
    const [idx, len] = isNumber(ssrItemCount) ? [0, ssrItemCount - 1] : ssrItemCount;
    const ssrItems = [];
...
export default <O extends HTMLElement = HTMLElement, I extends HTMLElement = O>({
    itemCount,
    ssrItemCount,
    itemSize = 50,
    horizontal,
    ...
    const [items, setItems] = useState<Item[]>(() => getInitItems(itemSize, ssrItemCount));
    ...
    const measureItems = useCallback((useCache = true) => {
...
//        console.log('tm: measureItems', msDataRef.current);
    ...
    const getCalcData = useCallback((scrollOffset: number) => {
        ...
        if (hasDynamicSizeRef.current) {
            ...
            vStart = findNearestBinarySearch(0, lastIdx, scrollOffset, (idx) => msData[idx].start);
        }
        ...
        const totalSize = Math[oStop < lastIdx ? "max" : "min"](msData[oStop].end + msData[oStop].size, msData[lastIdx].end);
    ...
    const scrollTo = useCallback((offset: number, isScrolling = true) => {
        if (outerRef.current) {
    ...
    const scrollToOffset = useCallback<ScrollTo>((val, cb) => {
        const { offset, smooth }: ScrollToOptions = isNumber(val) ? { offset: val } : val;
        if (!isNumber(offset)) {
            return;
        }
    ...
    const scrollToItem = useCallback<ScrollToItem>((val, cb) => {
        const { index, align = Align.auto, smooth, }: ScrollToItemOptions = isNumber(val) ? { index: val } : val;
        if (!isNumber(index)) {
            return;
        }
        ...
        if (align === Align.start || (align === Align.auto && scrollOffset + outerSize > end && scrollOffset > start)) {
    ...
    const [resetIsScrolling, cancelResetIsScrolling] = useDebounce(() => handleScroll(scrollOffsetRef.current), 150);
